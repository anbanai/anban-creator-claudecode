...
- Bash 执行 `echo $ANBANWRITER_DEFAULT_PROJECT`；非空 → `$PROJECT_ID`。为空 → `list_projects(platform="ecommerce")`，唯一匹配直接用，多个按品类/品牌语义匹配或让用户选。
- `get_project_profile(project_id="$PROJECT_ID", scope="ecommerce", task_id="$TASK_ID")` → 品牌定位/受众/参考图风格/**`image_strategy` provider 策略**。`task_id` 必传以继承模板派生风格。
- 读取任务输入：产品图路径 `$PRODUCT_PHOTOS`、`selected_modules`、`target_platform`、`selling_points`、`visual_style`、语言、`provider_strategy_override`。**产品图为空 → 停止并请求上传。**
...
- 对每张产品图 `analyze_image` 抽取电商转化属性
- 汇总锁定规格到 `$DIR/product-bible.md`
- 选出最佳锚点 `$ANCHOR_REF`
...
- 锚点优先（主图①先确立基准）→ 按模块逐张生成（产品档案前缀块 + provider 策略 + `verify_with_vision` 自检 + 3 轮收敛）
...
| 4 | `ecommerce-copywriting` | `copywriting.md` |
