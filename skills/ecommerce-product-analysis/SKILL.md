...
把 `$PRODUCT_PHOTOS` 写入 `$DIR/product-photos.md`，逐张标注序号、来源路径、可见性（是否可访问/是否需压缩）。任一不可访问剔除并记录，**至少保留 1 张**；全不可访问则报错停止。

### 步骤 2：逐张抽取产品属性

对每张产品图调 `analyze_image`，使用下方**电商属性抽取 prompt**。一张图可能只展示部分属性（如包装图只见包装文字，场景图见使用语境），多张互补。
...
- `$DIR/product-bible.md`（锁定规格）
- `$DIR/product-photos.md`（产品图清单与可见性）
