...
- **高分**：封面主视觉与标题钩子一致、配图有信息量/视觉多样性（复用 `article-visual-design` / `article-cover-design` 的 vision 校验结论）。
- **低分**：封面与标题各说各话、配图雷同/纯装饰。
- **证据**：`cover-prompt.md` + `images.json`。

### 7. 互动诱因

- **高分**：转发/收藏/评论诱因各 ≥1 且与正文自然连接、合规。
- **低分**：无诱因 / 靠违规诱导（抽奖/关注换资料，一票否决）。
