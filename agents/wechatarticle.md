...
Call `update_task_progress(task_id=$TASK_ID, stage="cover", title="封面生成", description="三维风格分析、封面生成与配图规划")`。
...
Call `update_task_progress(task_id=$TASK_ID, stage="illustration", title="插图生成", description="逐章节生成内容配图并验证质量")`。
...
Call `update_task_progress(task_id=$TASK_ID, stage="html", title="HTML转换", description="将Markdown文章转换为微信HTML格式")`。
