...
| **视觉模板** | 根据文章结构特征自动选 `claudecode/templates/article/*.yaml`（listicle / tutorial / story-narrative / long-form-essay），模板定义节奏、配图数量、layout module |
...
**步骤 2b：选题爆款策略锚定**（using the `article-viral-strategy` skill）
...
**爆款写作硬性要求**（using the `article-viral-strategy` skill，与上述既有要求并列追加）：
...
先 **using the `humanizer` skill** 对 `$DIR/03-article.md` 全文执行去 AI 改写：扫描 33 类 AI 写作模式（意义拔高、AI 高频词、三段式、否定排比、破折号滥用、空洞结尾等），按 draft → audit → final 流程改写。**改写而非删除**——覆盖原文全部信息点，保持段落数与字数量级，保留人称代入、情绪节奏与具体细节等人味。本步骤不调用任何 MCP 工具、不计费、无强度档位。改写产物保存为 `$DIR/04-article-final.md`。

再 using the `content-writing` skill 对 `$DIR/04-article-final.md` 执行违禁词合规检查，输出检查报告，并创建 `$DIR/content-quality-report.md`，逐项检查：
...
using the article-visual-design skill 完成以下五个子步骤。详细规范见 `skills/article-visual-design/SKILL.md` 与 `skills/article-visual-design/references/{cover,content,rhythm}.md`。
...
| 特征 | 模板 | YAML 路径 |
|------|------|-----------|
| 含"3 个/5 种/N 条" + 并列项 | `listicle` | `claudecode/templates/article/listicle.yaml` |
| 按步骤序号组织（步骤 1 / step N） | `tutorial` | `claudecode/templates/article/tutorial.yaml` |
| 情节弧、场景、人物时间线 | `story-narrative` | `claudecode/templates/article/story-narrative.yaml` |
| 其他（深度观点、评论、分析） | `long-form-essay`（默认） | `claudecode/templates/article/long-form-essay.yaml` |
...
**封面设计已独立成稿——完整方法论（官方比例 900×383/2.35:1、中心安全区、受控文字策略、概念推导、封面质量评分卡、迭代闭环、cover-prompt.md 审计）见 `article-cover-design` skill**。下方为本步骤关键调用要点。
...
using the content-writing skill 渲染 HTML。**不再使用 `convert_markdown` 自由发挥**，改用新的 `render_template` MCP 工具：
...
**审阅闭环**：任一项审阅未通过时标记待调整，自动回到对应步骤（正文、标题摘要、互动诱因、视觉 prompt 或 HTML 渲染）修订并重新审阅；不得调用 `publish_draft`。`viral-audit.md` 未通过（整体 <7.0 或任一硬性必过项不通过）同样先调整再复审。
...
- **模板驱动节奏**（硬性要求）：从 `claudecode/templates/article/*.yaml` 加载模板，不得临时编造节奏
- **节奏规划完整**（硬性要求）：`visual-rhythm-plan.md` 存在且每个 `##` 都映射到 slot
- 封面图必须成功生成并上传（硬性要求，**仅封面开关开启时**）
- **封面质量闸门**（硬性要求，**仅封面开关开启时**）：`cover-prompt.md` 必须含 `final_title`、`digest_hook`、`cover_hook`、`visual_metaphor`、`thumbnail_strategy`、`anti_generic_constraints`、`visual_quality_scorecard`，且 `visual_quality_scorecard.overall_pass=true`
...
- [ ] `$DIR/cover-prompt.md` 存在，含 `2.35:1` 比例、三维风格来源、`final_title`、`digest_hook`、`cover_hook`、`visual_metaphor`、`thumbnail_strategy`、`anti_generic_constraints`、`required_entities`、`visual_quality_scorecard`、vision 校验结果
...
- [ ] **封面质量评分卡未通过**（`visual_quality_scorecard.overall_pass=false`，或低对比、泛水墨模板感、标题/封面/digest 不协同、系列辨识度低）→ 先改 `cover_hook` / `visual_metaphor` / `thumbnail_strategy` 再重试
...
- [ ] **`$DIR/cover-prompt.md` 缺失或无 `visual_quality_scorecard`** → 步骤 6d 第 9 步必须原子写入
...
- [ ] **`viral-audit.md` 整体 <7.0 或硬性必过项任一失败** → 不得发布，按回退路径回步骤 3/5
