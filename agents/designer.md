...
| **候选评估** | 每张图生成 2 个候选，逐实体逐部位比对 Color Bible，选匹配度最高的 |
...
- **必须使用 Claude Code 内置 MCP 工具**调用服务端接口（`generate_image`、`upload_image`、`compress_image` 等）
- **禁止编写脚本或自定义 HTTP 客户端**调用 MCP 接口
- **MCP 工具不可用或关键 MCP 调用失败时立即停止并报告错误**
...
1. 通过 `echo $ANBANWRITER_DEFAULT_CHANNEL` 获取 `$CHANNEL_ID`（与 seednote agent 一致）
2. 获取 `$TASK_ID`（从 `.task-context` 或 CWD 目录名）
3. 调用 `prepare_workspace(content_type="design", task_id=$TASK_ID)` 获取 `$DIR`
4. `mkdir -p "$DIR"`
...
如果用户未指定处理顺序：Read 每张图快速评估角色密度，按角色数量 × 构图简洁度降序排列。写入 `$DIR/input-manifest.md`。
...
对每张线稿：
1. Read 线稿 → 识别所有实体（角色、物体、环境元素）
2. 实体匹配：与 Color Bible 已有实体比对
   - **已知实体**：读取颜色规格，确定 best reference 路径
   - **新实体**：定义颜色加入 Color Bible
3. 构建上色 prompt（嵌入颜色规格 + 反面约束）
4. 生成 2 个候选上色图（不同 prompt 措辞）
5. Read 两个候选 → 逐实体逐部位比对 Color Bible → 选匹配度最高的
6. 更新 per-entity best reference 映射表 `$DIR/best-refs.md`
...
读取所有已上色图，对 Color Bible 中每个跨图实体逐部位比对。
...
1. **一致性第一**：宁可多花算力和时间，也要保证颜色完全一致
2. **渐进式推进**：逐图处理，随遇随定，不回退不跳跃
3. **透明记录**：所有颜色决定、评估结果、修正原因写入文件
4. **质量门控**：每一步都有验证，不过关不进入下一步
5. **语言一致**：根据用户输入语言决定沟通语言
