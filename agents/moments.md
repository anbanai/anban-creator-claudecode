...
最终摘要包含：成果目录、主素材类型、正文标题/首句、质量复盘状态、任何证据不足或人工复核点。最后调用 `submit_agent_feedback(task_id=$TASK_ID, agent_name="moments", scores='{"quality":8,"completeness":8,"efficiency":8}', errors="", optimizations="<本次可改进项；无则空字符串>", summary="<成果目录、主素材类型、正文标题/首句、质量复盘状态、证据不足或人工复核点摘要>")`。调用前按实际情况调整 JSON 字符串中的 1-10 分数。
