...
7. **视频合成（可选）**：生成图片后如用户要求生成视频版本，using the video-composition skill，将所有图片（封面 + 内容图 + 尾图）组装为 MP4 视频，保存到 `$DIR/video.mp4`
...
8. **视频合成（可选）**：生成图片后如用户要求生成视频版本，using the video-composition skill，将所有图片（封面 + 内容图 + 尾图）组装为 MP4 视频，保存到 `$DIR/video.mp4`
...
- 图片文件均存在且可访问（≥3 张）
- 正文中不含诱导互动表述（无"评论区"、"点赞收藏"、"领取"、"抽奖"等引导语）

---

## 诱导互动合规规则

**背景**：小红书平台对"诱导互动"行为严格限流，多次违规将影响账号流量和商业权益。以下规则覆盖平台已确认的违规类别。

### 禁止模式

| 违规类别 | 禁止表述（示例） | 说明 |
|---------|-----------------|------|
| **直接引导评论** | "评论区聊聊"、"评论区告诉我"、"评论区选一个"、"评论区见" | 显性引导用户在评论区完成特定互动行为 |
| **条件性互动引导** | "点赞过100更新"、"收藏防走丢"、"收藏不迷路"、"关注才能看下期" | 以互动行为作为获取内容的条件 |
| **福利诱导** | "评论领取"、"关注送模板"、"私信发你资料"、"扣1领取" | 以资源/物品交换互动行为 |
| **抽奖诱导** | "关注+评论参与抽奖"、"点赞抽送XX"、"转发抽奖"、"送福利" | 以抽奖噱头诱导互动 |
| **互关互赞** | "互关必回"、"关注回粉"、"互赞"、"一起涨粉" | 以互惠形式诱导涨粉 |
| **教学性引导** | "双击屏幕"、"长按点赞"、"记得点赞收藏"、"一键三连" | 指导用户完成特定互动操作 |

### 合规替代

| 原意图 | 合规写法 | 原则 |
|--------|---------|------|
| 引导评论讨论 | "你们平时是怎么做的？"（开放式问题，无"评论区"字样） | 提出问题但不指定互动渠道 |
| 引导收藏 | 正文中放入清单/步骤/对比表（天然收藏诱饵，不需要引导语） | 内容本身驱动收藏，无需语言引导 |
| 暗示系列内容 | "下次聊XX"（无条件、无互动门槛） | 自然预告，不以互动为条件 |
| 分享价值 | "建议收藏备用"（最多用一次，且不带"不迷路/防走丢"） | 中性提示，不构成诱导 |

### 写作红线

- 正文中**禁止出现"评论区"三个字**
- 正文中**禁止出现"点赞收藏"四字组合**（单独出现"点赞"或"收藏"不违规，但组合使用属于教学性引导）
- 正文结尾**禁止以"👇"指向评论区**
- 尾图中**禁止引导互关互赞**
...
| **诱导互动表述未清除** | 合规检查阶段增加诱导互动专项扫描，自动删除/改写违规表述 |
