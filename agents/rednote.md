...
**小红书对"诱导互动"行为严格限流，多次违规将影响账号流量和商业权益。** 完整禁止模式列表和合规替代详见 `rednote-writing` skill（`prohibited-words.md` 第三节）。

写作时必须遵守以下红线，合规检查阶段由 `rednote-writing` skill 的 9.5 节执行专项扫描：
- 正文中**禁止出现"评论区"三个字**
